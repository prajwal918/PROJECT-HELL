...
def check_dependencies():
    ...
    required = ['asyncio', 'websockets', 'requests', 'python-dotenv', 'pytz']
    missing = []
...
def check_nova_files():
    print("\nChecking NOVA files...")
    nova_dir = Path(__file__).parent
    required = [
        'main.py',
        'config.py',
        'core/event_whitelist.py',
        'core/directional_bias.py',
        'core/l3_gate.py',
    ...
    if env_file.exists():
        ...
        with open(env_file) as f:
            ...
            if 'FRED_API_KEY' in content:
                print(f"[PASS] FRED_API_KEY configured")
            else:
                print(f"[FAIL] FRED_API_KEY not configured")
            return True
    else:
...
def main():
    ...
    if passed == total:
        print("\n[PASS] All checks passed! NOVA is ready to start.")
        print("\nTo start NOVA:")
